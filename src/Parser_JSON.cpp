...
bool Parser_JSON::parseJsonFile(const QString &file_path)
{
    ...
    // Дополнительно: загрузка функций / стеки для работы: поддержка "call"
                          ...
                          int clause_id)
{
    ...
    p.setBit(rootEl->getPosition()); // TO root (positive)
    n.setBit(leafEl->getPosition()); // FROM leaf (negative)
...
void Parser_JSON::processCallRow(const QJsonObject &row_obj, Tree *&current_tree, int call_clause_id)
{
    ...
    for (const QJsonValue &rv : funcRows)
    {
        ...
        QJsonObject rr = substituteParams(r, params).toObject();
                                    ...
                                    int nesting_level)
{
    ...
    // Переходим по полю и получаем новую переменную
                                         ...
                                         const QString &operation_type)
{
    ...
    // Удаляем связи с корнем для выделяемой переменной
    removeRootLinkClauses(current_tree, current_tree->getName(), var_name);

    // Удаляем предыдущие исходящие соединения (переприсваивание)
    // + если раньше var_name указывал на MEMORY, чистим старый блок памяти
    ...
    {
        QString oldTarget = prev_conn.getTo();
        ...
        TreeElement *oldEl = current_tree->findElement(oldTarget);
        if (oldEl && oldEl->isMemory())
        {
        }
    }

    // Создаем новый MEMORY элемент
    ...
    // Удаляем связи с корнем для памяти
    removeRootLinkClauses(current_tree, current_tree->getName(), mem_name);

    // Создаем связь var -> mem
    {
        BoolVector pos_vec(max_bytes_count_ * 8);
        BoolVector neg_vec(max_bytes_count_ * 8);
        pos_vec.setBit(mem_element->getPosition()); // TO memory
        neg_vec.setBit(element->getPosition());     // FROM var
        Connection c(var_name, mem_name, pos_vec, neg_vec, clause_id);
        current_tree->addConnection(c);
    }

    // --- создаём поля left/right ---
    auto ensureField = [&](const QString &suffix, int typeEnum) -> QString
    {
        ...
        if (!field_el)
        {
            ...
            p.setBit(field_el->getPosition());    // TO field
            n.setBit(mem_element->getPosition()); // FROM mem
            Connection c(mem_name, field_name, p, n, clause_id);
            current_tree->addConnection(c);
    ...
    // --- ВАЖНО: если у mem есть поля, связь mem -> root НЕ нужна. Нужны leaf->root для полей ---
    // На всякий случай убираем mem->root, если вдруг была
    current_tree->removeConnection(mem_name, current_tree->getName());

    // Добавляем leaf->root для M#.left и M#.right (как раньше)
...
void Parser_JSON::removeMemoryKeepChildren(Tree *t, const QString &memName)
{
    ...
    // Сохраняем дочерние MEMORY, на которые указывали поля (их НЕ удаляем)
    auto collectChildMems = [&](const QString &field)
    {
        QList<Connection> outs = t->getConnectionsFrom(field);
        for (const Connection &c : outs)
        {
            TreeElement *toEl = t->findElement(c.getTo());
            if (toEl && toEl->isMemory())
            {
                // ничего не делаем: просто "замечаем", что оно есть
                // (удалять не будем)
            }
        }
    };
    ...
    // Удаляем все связи от/к указанного узла
    ...
    // Удаляем элементы полей и память
    t->removeElement(L);
    t->removeElement(R);
...
void Parser_JSON::handleFree(Tree *current_tree, const QString &var_name)
{
    ...
    // 1) Удаляем связи var_name -> root (лист к корню), если были (пересоздадим ниже)
    removeRootLinkClauses(current_tree, current_tree->getName(), var_name);

    // 2) Удаляем исходящие связи var_name -> * (это и есть "обнулить указатель"),
    //    а если var_name указывала на MEMORY — удаляем MEMORY-блок (но НЕ его дочерние MEMORY)
    ...
    for (const Connection &conn : outgoing_connections)
    {
        ...
        {
            // Удаляем memory и его поля, но НЕ трогаем memory, на которые могли указывать поля
    ...
    {
        const bool is_struct_link = is_field && (conn.getFrom() == parent_name);
    ...
    // 4) ВАРИАНТ 1: NULL = отсутствие исходящей связи.
    //    После free поле становится листом => нужно снова дать leaf->root связь.
    auto ensureLeafToRoot = [&](const QString &leaf_name)
    {
        ...
        for (const Connection &c : outs)
        {
            if (c.getTo() == current_tree->getName())
                return;
        }

        BoolVector p(max_bytes_count_ * 8);
        BoolVector n(max_bytes_count_ * 8);
        p.setBit(root->getPosition());    // TO root
        n.setBit(element->getPosition()); // FROM leaf

        // clause_id здесь неизвестен (handleFree без clause_id), ставим -1
        Connection c(leaf_name, current_tree->getName(), p, n, -1);
        current_tree->addConnection(c);
    ...
    if (is_field)
    {
        // поле НЕ удаляем — это часть структуры
    ...
    {
        // Обычную переменную можно удалить как раньше (если это не корень дерева)
...
// удалить “блок памяти” и его поля (минимально: связи + элементы)
